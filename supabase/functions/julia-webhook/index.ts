...
/**
 * julia-webhook: Wrapper for julIA agent
 * 
 * This is a lightweight wrapper that forwards requests to z-api-webhook
 * with the agent_id set to 'julia'. The z-api-webhook handles message
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[julia-webhook] Forwarding request to z-api-webhook with agent_id: julia');
    ...
    console.log('[julia-webhook] Response status:', response.status);
    ...
    console.error('[julia-webhook] Error:', error);
