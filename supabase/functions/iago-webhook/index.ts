...
/**
 * iago-webhook: Wrapper for IAgo agent
 * 
 * This is a lightweight wrapper that forwards requests to z-api-webhook
 * with the agent_id set to 'iago'. The z-api-webhook handles message
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[iago-webhook] Forwarding request to z-api-webhook with agent_id: iago');
    ...
    console.log('[iago-webhook] Response status:', response.status);
    ...
    console.error('[iago-webhook] Error:', error);
